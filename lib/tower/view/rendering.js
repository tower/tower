...
Tower.View.Rendering = {
  ...
  _renderString: function(string, options, callback) {
    ...
    if (!!options.type.match(/coffee/)) {
      ...
      try {
        ...
        locals.format = true;
        locals.hardcode = _.extend({}, Tower.View.ComponentHelper, Tower.View.AssetHelper, Tower.View.HeadHelper, Tower.View.RenderingHelper, {
  ...
  renderWithEngine: function(template, engine) {
    return require("coffeekup").render(require("shift").engine(engine || "coffee").render(template), this);
  }
};
