...
Tower.Store.MongoDB.Serialization = {
  ...
  serializeQuery: function(record) {
    var field, key, operator, query, result, schema, value, _key, _value;
    schema = this.schema();
    result = {};
    query = this.deserializeModel(record);
    for (key in query) {
      value = query[key];
      field = schema[key];
      if (key === "id") key = "_id";
      if (_.isRegExp(value)) {
        ...
        for (_key in value) {
          ...
          if (operator === "$eq") {
            ...
            if (_key === "$in") _value = Tower.Support.Object.toArray(_value);
  ...
  encodeArray: function(value, operation) {
    if (!(operation || value === null || Tower.Support.Object.isArray(value))) {
