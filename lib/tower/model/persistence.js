...
Tower.Model.Persistence = {
  ...
  _create: function(callback) {
    ...
    this.runCallbacks("create", function(block) {
      var complete;
      complete = _this._callback(block, callback);
      return _this.constructor.instantiate(false).create(_this, function(error) {
  ...
  _update: function(updates, callback) {
    ...
    this.runCallbacks("update", function(block) {
      var complete;
      complete = _this._callback(block, callback);
      return _this.constructor.instantiate(false).update(_this.get("id"), updates, function(error) {
        if (error && !callback) throw error;
        if (!error) {
          _this._resetChanges();
          _this.persistent = true;
  ...
  _destroy: function(callback) {
    ...
    this.runCallbacks("destroy", function(block) {
      var complete;
      complete = _this._callback(block, callback);
      return _this.constructor.instantiate(false).destroy(_this, function(error) {
        if (error && !callback) throw error;
        if (!error) {
