...
module.exports = function(grunt) {
  ...
  grunt.registerMultiTask('templates', 'Compile templates', function() {
    var file, files, iterator, name, result, taskDone, template, _i, _len,
      _this = this;
    name = "";
    taskDone = this.async();
    ...
    for (_i = 0, _len = files.length; _i < _len; _i++) {
      file = files[_i];
      if (!file.match(/app\/templates\/.+\.coffee$/)) {
        continue;
      }
      if (!file.match(/\.coffee$/)) {
        continue;
      }
      if (file.match('layout/application')) {
        continue;
      }
    ...
    iterator = function(item, next) {
      ...
      name = item[0].replace(/app\/templates\/(?:client|shared)\//, '');
      fileName = name.split('/');
  ...
  grunt.registerHelper('bundleDependencies', function(done) {
    ...
    require(towerPath);
    fs = require('fs');
    JAVASCRIPTS = ['underscore', 'underscore.string', 'moment', 'geolib', 'validator', 'accounting', 'inflection', 'async', 'socket.io', 'handlebars', 'ember', 'tower', 'bootstrap/bootstrap-dropdown'];
    bundlePath = null;
    processEach = function(keys, bundle, next) {
      ...
      iterator = function(local, nextDownload) {
        ...
        local = local + '.js';
        return fs.readFile("./dist/" + local, 'utf-8', function(error, content) {
  ...
  return grunt.registerMultiTask('build', 'Build tower for the client', function() {
    ...
    buildIt = function() {
      ...
      return mint.coffee(content, {
        ...
        return fs.writeFileSync("./dist/tower.js", result);
      });
    };
    return buildIt();
  });
