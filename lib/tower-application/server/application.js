...
Tower.Application = (function(_super) {
  ...
  Application.reopenClass({
    _callbacks: {},
    autoloadPaths: ['app/helpers', 'app/concerns', 'app/models', 'app/controllers', 'app/presenters', 'app/mailers', 'app/middleware', 'app/jobs'],
  ...
  Application.reopen({
    ...
    configureStores: function(configuration, callback) {
      ...
      return Tower.parallel(databaseNames, iterator, function() {
        if (!defaultStoreSet) {
          console.warn('Default database not set, using Memory store');
        }
        if (callback) {
    ...
    initializeConsoleHooks: function() {
      ...
      this.hook = hook = hookio.createHook({
        name: 'tower-console',
        silent: true
      });
      hook.start();
      return Tower.notifyConnections = function(action, records, callback) {
        if (!(records instanceof Array)) {
          records = [records];
        }
        return hook.emit('notifyConnections', {
          action: action,
    ...
    initializeServerHooks: function() {
      ...
      hook.on('tower-console::notifyConnections', function(data) {
        var klass, records, store;
        if (data.type) {
          klass = Tower.constant(data.type);
          store = klass.store();
          records = _.map(JSON.parse(data.records), function(attributes) {
            return store.serializeModel(attributes, true);
          });
    ...
    _loadPreinitializers: function() {
      return this._requirePaths(this._selectPaths('app/config', 'preinitializers'));
    },
    ...
    _loadLocales: function() {
      ...
      _ref = this._selectPaths('app/config', 'locales');
      _results = [];
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        path = _ref[_i];
    ...
    _loadInitializers: function() {
      return this._requirePaths(this._selectPaths('app/config', 'initializers'));
    },
    _loadMVC: function() {
      var path, _i, _len, _ref, _results;
      ...
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        ...
        _results.push(this._requirePaths(this._selectPaths(path)));
      }
      return _results;
    },
    ...
    _selectPaths: function(pathStart, pathEnd, type) {
