  ...
  Coach.Controller.Events = {
    ClassMethods: {
      ...
      socketNamespace: function() {
        return Coach.Support.String.pluralize(Coach.Support.String.camelize(this.name.replace(/(Controller)$/, ""), false));
      },
      addSocketEventHandler: function(name, handler, options) {
        var _this = this;
        if (!this.io) {
          this._socketHandlers = {};
          this.io = Coach.Application.instance().socket.of(this.socketNamespace()).on("connection", function(socket) {
