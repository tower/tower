(function() {
  ...
  Metro.Store.MongoDB = (function() {
    ...
    MongoDB.prototype.find = function(query, callback) {
      var self;
      self = this;
      this.collection().find(this._translateQuery(query)).toArray(function(error, docs) {
        var doc, _i, _len;
        if (!error) {
    ...
    MongoDB.prototype._translateQuery = function(query) {
      ...
      try {
        if (query.id) {
          result["_id"] = this.constructor.database.bson_serializer.ObjectID(query.id.toString());
        }
      } catch (error) {
        result["_id"] = query.id;
      }
