(function() {
  ...
  Metro.Store.Memory = (function() {
    ...
    Memory.prototype.first = function(query, callback) {
      var result;
      result = this.find(query, function(error, records) {
        if (callback) return callback.call(this, error, records[0]);
      });
      return result[0];
    };
    ...
    Memory.prototype.last = function(query, callback) {
      ...
      result = this.find(query, function(error, records) {
        if (callback) {
          return callback.call(this, error, records[records.length - 1]);
        }
      });
    ...
    Memory.prototype.length = function(query, callback) {
      return this.find(query, function(error, records) {
        if (callback) return callback.call(this, error, records.length);
      }).length;
    };
    ...
    Memory.prototype.create = function(attributes, callback) {
      var _ref;
      if ((_ref = attributes.id) == null) attributes.id = this.generateId();
      return this.records[attributes.id] = this.serializeAttributes(attributes);
    };

    Memory.prototype.update = function(query, attributes, callback) {
      return this.find(query, function(error, records) {
