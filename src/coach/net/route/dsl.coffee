class Coach.Net.Route.DSL
  constructor: ->
    @_scope = {}

  match: ->
    @scope ||= {}
    Coach.Net.Route.create(new Coach.Net.Route(@_extractOptions(arguments...)))

  get: ->
    @matchMethod("get", Coach.Support.Array.args(arguments))

  post: ->
    @matchMethod("post", Coach.Support.Array.args(arguments))

  put: ->
    @matchMethod("put", Coach.Support.Array.args(arguments))

  delete: ->
    @matchMethod("delete", Coach.Support.Array.args(arguments))
  ...
  matchMethod: (method, args) ->
    ...
    options.name    = name
    if @_scope.name
      options.name = @_scope.name + Coach.Support.String.camelize(options.name)
  ...
  scope: (options = {}, block) ->
    ...
    @_scope = Coach.Support.Object.extend {}, originalScope, options
    block.call(@)
  ...
  * @param {String} path
  ...
  namespace: (path, options, block) ->
    ...
    options = Coach.Support.Object.extend(name: path, path: path, as: path, module: path, shallowPath: path, shallowPrefix: path, options)
  ...
  resource: (name, options = {}) ->
    ...
    @match "#{name}/new", Coach.Support.Object.extend({action: "new"}, options)
    @match "#{name}", Coach.Support.Object.extend({action: "create", method: "POST"}, options)
    @match "#{name}/", Coach.Support.Object.extend({action: "show"}, options)
    @match "#{name}/edit", Coach.Support.Object.extend({action: "edit"}, options)
    @match "#{name}", Coach.Support.Object.extend({action: "update", method: "PUT"}, options)
    @match "#{name}", Coach.Support.Object.extend({action: "destroy", method: "DELETE"}, options)
  ...
  resources: (name, options, callback) ->
    ...
    if @_scope.name
      many = @_scope.name + Coach.Support.String.camelize(name)
    else
      many = name

    one   = Coach.Support.String.singularize(many)

    @match "#{path}", Coach.Support.Object.extend({name: "#{many}", action: "index"}, options)
    @match "#{path}/new", Coach.Support.Object.extend({name: "new#{Coach.Support.String.camelize(one)}", action: "new"}, options)
    @match "#{path}", Coach.Support.Object.extend({action: "create", method: "POST"}, options)
    @match "#{path}/:id", Coach.Support.Object.extend({name: "#{one}", action: "show"}, options)
    @match "#{path}/:id/edit", Coach.Support.Object.extend({name: "edit#{Coach.Support.String.camelize(one)}", action: "edit"}, options)
    @match "#{path}/:id", Coach.Support.Object.extend({action: "update", method: "PUT"}, options)
    @match "#{path}/:id", Coach.Support.Object.extend({action: "destroy", method: "DELETE"}, options)

    if callback
      @scope Coach.Support.Object.extend({path: "#{path}/:#{Coach.Support.String.singularize(name)}Id", name: one}, options), callback
    @
  ...
  root: (options) ->
    @match '/', Coach.Support.Object.extend(as: "root", options)
  ...
  _extractConstraints: (options) ->
    Coach.Support.Object.extend(@_scope.constraints || {}, options.constraints || {})
  ...
  _extractController: (options = {}) ->
    ...
    name: controller, action: action, className: Coach.Support.String.camelize("#{controller}")
      
module.exports = Coach.Route.DSL
