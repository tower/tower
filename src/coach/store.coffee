...
class Coach.Store extends Coach.Object
  ...
  serializeModel: (attributes) ->
    klass = Coach.constant(@className)
  ...
  constructor: (options = {}) ->
    @name       = options.name
    @className  = options.className || Coach.namespaced(Coach.Support.String.camelize(Coach.Support.String.singularize(@name)))
  ...
  schema: ->
    Coach.constant(@className).schema()
