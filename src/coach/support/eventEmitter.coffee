Coach.Support.EventEmitter =
  ...
  hasEventListener: (key) ->
    Coach.Support.Object.isPresent(@events(), key)
    
  event: (key) ->
    @events()[key] ||= new Coach.Event(@, key)
  ...
  fire: (key) ->
    event = @event(key)
    event.fire.call event, Coach.Support.Array.args(arguments, 1)
    
  allowAndFire: (key) ->
    @event(key).allowAndFire(Coach.Support.Array.args(arguments, 1))
