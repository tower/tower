Coach.Store.MongoDB.Serialization =
  ...
  encode: (field, value, operation) ->
    ...
    value = [value] if operation == "$in" && !Coach.Support.Object.isArray(value)
    value
  ...
  encodeDate: (value) ->
    # if Coach.Support.config.useTimeZone
    time = require('moment')
    switch typeof(value)
      when "string"
        time.parse(value)
  ...
  encodeArray: (value) ->
    unless value == null || Coach.Support.Object.isArray(value)
      throw new Error("Value is not Array")
    value
    
  encodeFloat: (value) ->
    return null if Coach.Support.Object.isBlank(value)
    try
      parseFloat(value)
    catch error
      value
...
module.exports = Coach.Store.MongoDB.Serialization
