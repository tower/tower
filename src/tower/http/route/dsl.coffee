class Tower.HTTP.Route.DSL
  ...
  resource: (name, options = {}) ->
    ...
    @match "#{name}/new", _.extend({action: "new"}, options)
    @match "#{name}", _.extend({action: "create", method: "POST"}, options)
    @match "#{name}/", _.extend({action: "show"}, options)
    @match "#{name}/edit", _.extend({action: "edit"}, options)
    @match "#{name}", _.extend({action: "update", method: "PUT"}, options)
    @match "#{name}", _.extend({action: "destroy", method: "DELETE"}, options)
